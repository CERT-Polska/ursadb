...
QueryResult OnDiskIndex::query(const QueryGraph &graph,
    ...
    QueryFunc oracle = [this, counters](uint64_t raw_gram) {
        ...
        if (gram) {
            return QueryResult(std::move(SortedRun(
...
std::vector<uint8_t> OnDiskIndex::get_run(uint64_t ptr,
...
std::vector<uint8_t> OnDiskIndex::query_primitive(TriGram trigram,
